...
pages = []
for i in range(4):
    pages.append(tk.Frame(root, bg="#1C1C1C"))
...
def basicPageSetup(pageNo):
    ...
    back_button = tk.Button(pages[pageNo], image=back_img, bg="#1C1C1C", bd=0, highlightthickness=0,
                            activebackground="#1C1C1C", command=goBack)
    back_button.image = back_img
    back_button.place(x=10, y=0)

    heading = tk.Label(pages[pageNo], fg="white", bg="#1C1C1C", font="Arial 20 bold", pady=10)
    heading.pack()

    content = tk.Frame(pages[pageNo], bg="#1C1C1C", pady=20)
    content.pack(expand="true", fill="both")

    left_frame = tk.Frame(content, bg="#1C1C1C")
    left_frame.grid(row=0, column=0, sticky="nsew")

    right_frame = tk.LabelFrame(content, text="Detected Criminals", bg="#1C1C1C", font="Arial 20 bold", bd=4,
                                foreground="#E31C25", labelanchor="n")
...
def showImage(frame, img_size):
    ...
    if (img_label == None):
        img_label = tk.Label(left_frame, image=img, bg="#1C1C1C")
        img_label.image = img
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if (len(path_list) < 5):
        ...
        slide_control_panel = tk.Frame(left_frame, bg="#1C1C1C", pady=20)
        slide_control_panel.pack()

        back_img = tk.PhotoImage(file="previous.png")
        next_img = tk.PhotoImage(file="next.png")

        prev_slide = tk.Button(slide_control_panel, image=back_img, bg="#1C1C1C", bd=0, highlightthickness=0,
                               activebackground="#1C1C1C", command=lambda: getNewSlide("prev"))
        ...
        slide_caption = tk.Label(slide_control_panel, text="Image 1 of {}".format(len(img_list)), fg="#ff9800",
                                 bg="#1C1C1C", font="Arial 15 bold")
        slide_caption.grid(row=0, column=1)

        next_slide = tk.Button(slide_control_panel, image=next_img, bg="#1C1C1C", bd=0, highlightthickness=0,
                               activebackground="#1C1C1C", command=lambda: getNewSlide("next"))
...
def getPage1():
    ...
    btn_grid = tk.Frame(left_frame, bg="#1C1C1C")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var),
              font="Arial 15 bold", bg="#E31C25",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#BB000E",
              activeforeground="white").grid(row=0, column=0, padx=25, pady=25)

    # Creating Scrollable Frame
    canvas = tk.Canvas(right_frame, bg="#1C1C1C", highlightthickness=0)
    canvas.pack(side="left", fill="both", expand="true", padx=30)
    scrollbar = tk.Scrollbar(right_frame, command=canvas.yview, width=20, troughcolor="#1C1C1C", bd=0,
                             activebackground="#00bcd4", bg="#E31C25", relief="raised")
    scrollbar.pack(side="left", fill="y")

    scroll_frame = tk.Frame(canvas, bg="#1C1C1C", pady=20)
    ...
    tk.Label(scroll_frame, text="* Required Fields", bg="#1C1C1C", fg="yellow", font="Arial 13 bold").pack()
    # Adding Input Fields
    ...
    for i, field in enumerate(input_fields):
        row = tk.Frame(scroll_frame, bg="#1C1C1C")
        row.pack(side="top", fill="x", pady=15)

        label = tk.Text(row, width=20, height=1, bg="#1C1C1C", fg="#ffffff", font="Arial 13", highlightthickness=0,
                        bd=0)
        ...
        if (i != ip_len - 1):
            ...
            opt_menu.configure(font="Arial 13", bg="#E31C25", fg="white", bd=0, highlightthickness=0,
    ...
    tk.Button(scroll_frame, text="Register", command=lambda: register(entries, required, menu_var),
              font="Arial 15 bold",
              bg="#E31C25", fg="white", pady=10, padx=30, bd=0, highlightthickness=0, activebackground="#BB000E",
...
def showCriminalProfile(name):
    ...
    for i, item in enumerate(crim_data.items()):
        tk.Label(info_frame, text=item[0], pady=15, fg="yellow", font="Arial 15 bold", bg="#1C1C1C").grid(row=i,
                                                                                                          column=0,
                                                                                                          sticky='w')
        tk.Label(info_frame, text=":", fg="yellow", padx=50, font="Arial 15 bold", bg="#1C1C1C").grid(row=i, column=1)
        val = "---" if (item[1] == "") else item[1]
        tk.Label(info_frame, text=val.capitalize(), fg="white", font="Arial 15", bg="#1C1C1C").grid(row=i, column=2,
                                                                                                    sticky='w')
...
def getPage2():
    ...
    tk.Button(btn_grid, text="Select Image", command=selectImage, font="Arial 15 bold", padx=20, bg="#E31C25",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#BB000E",
              activeforeground="white").grid(row=0, column=0, padx=25, pady=25)
    tk.Button(btn_grid, text="Recognize", command=startRecognition, font="Arial 15 bold", padx=20, bg="#E31C25",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#BB000E",
...
def selectVideo():
    ...
    if (len(path) > 0):
        ...
        thread = threading.Thread(target=testvideo, args=(model, names, path))
        thread.start()


## my code... not tested
def testvideo(model, names, video):
    global thread_event, left_frame, webcam, img_label
...
def getPage4():
    ...
    basicPageSetup(3)
    heading.configure(text="Video Criminal")
    right_frame.configure(text="Detected Criminals")
    left_frame.configure(pady=40)
    btn_grid = tk.Frame(left_frame, bg="#1C1C1C")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Video", command=selectVideo, font="Arial 15 bold", padx=20, bg="#E31C25",
              fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#BB000E",
              activeforeground="white").grid(row=0, column=0, padx=25, pady=25)


######################################## Home Page ####################################
tk.Label(pages[0], text="Suspect Recognition From CCTV Footages", fg="white", bg="#1C1C1C",
         font="Arial 35 bold", pady=30).pack()

logo_img = tk.PhotoImage(file="skit.png")
logo_button = tk.Button(pages[0], image=logo_img, bg="#1C1C1C",height=143,bd=0, highlightthickness=0)
logo_button.image = logo_img
logo_button.place(x=0, y=10)



log = tk.PhotoImage(file="detective.png")

tk.Label(pages[0], image=log, bg="#1C1C1C",height=200).pack()

btn_frame = tk.Frame(pages[0], bg="#1C1C1C", pady=5)
btn_frame.pack()

button1 = tk.Button(btn_frame, text="Register Criminal", command=getPage1)
button2 = tk.Button(btn_frame, text="Detect Criminal", command=getPage2)
button3 = tk.Button(btn_frame, text="Live Surveillance", command=getPage3)
button4 = tk.Button(btn_frame, text="Video Surveillance", command=getPage4)

for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 20", width=17, bg="#E31C25", fg="white",
                  pady=10, bd=0, highlightthickness=0, activebackground="#BB000E", activeforeground="white")
    btn.pack(pady=10)
